...
def load_yelp_minimal(
    ...
    if not review_path.exist():
        raise FileNotFoundError(f"Missing {review_path}. Please ensure the Yelp dataset is downloaded and placed in the specified data directory.")
    ...
    reviews = reviews.all[cols].copy()
    reviews["date"] = pd.to_datetime(reviews["date"], errors = "coerce", utc=True).dt.tz_convert(None)
    ...
    if user_path.exists():
        ...
        if "yelping_since" in users.columns:
            users["yelping_since"] = pd.to_datetime(users["yelping_since"], errors="coerce", utc=True).dt.tz_convert(None)
